  ...
  var stringParser = function (input) {
    ...
    if (input[0] === '"') {
      ...
      if (input.indexOf('\\') > 0 && input.indexOf('\\') < endIndex) input = input.replace('\\', '')
      return [input.slice(0, endIndex), input.slice(endIndex + 1, input.length)]
    }
    return null
  }
  ...
  var numberParser = function (input) {
    ...
    if (res = numRegex.exec(input)) {
      var resLen = res.index + res[0].length
      var num = parseFloat(res[0])
      return [num, input.slice(resLen)]
    }
    return null
  }
  ...
  var helperObjectParser = function (input, outArrObj) {
    if (input.length > 0 && input[0] !== '}') {
      ...
      var value = valueParser(spaceParser(input[1]))
